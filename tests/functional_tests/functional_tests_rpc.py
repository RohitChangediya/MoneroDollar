...
monerodollard_base = [builddir + "/bin/monerodollard", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--offline", "--no-igd", "--p2p-bind-port", "monerodollard_p2p_port", "--rpc-bind-port", "monerodollard_rpc_port", "--zmq-rpc-bind-port", "monerodollard_zmq_port", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--log-level", "1"]
monerodollard_extra = [
  [],
  ["--rpc-payment-address", "44SKxxLQw929wRF6BA9paQ1EWFshNnKhXM3qz6Mo3JGDE2YG3xyzVutMStEicxbQGRfrYvAAYxH6Fe8rnD56EaNwUiqhcwR", "--rpc-payment-difficulty", str(DIFFICULTY), "--rpc-payment-credits", "5000", "--data-dir", builddir + "/functional-tests-directory/monerodollard1"],
]
wallet_base = [builddir + "/bin/monerodollar-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--disable-rpc-login", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--daemon-port", "18180", "--log-level", "1"]
wallet_extra = [
]
...
for i in range(N_MONERODOLLARDS):
  command_lines.append([str(18180+i) if x == "monerodollard_rpc_port" else str(18280+i) if x == "monerodollard_p2p_port" else str(18380+i) if x == "monerodollard_zmq_port" else x for x in monerodollard_base])
  if i < len(monerodollard_extra):
    command_lines[-1] += monerodollard_extra[i]
  outputs.append(open(builddir + '/tests/functional_tests/monerodollard' + str(i) + '.log', 'a+'))
