...
Core tests take longer than any other MoneroDollar tests, due to the high amount of computational work involved in validating core components.
...
To run only MoneroDollar's core tests (after building):
...
To run only MoneroDollar's crypto tests (after building):
...
```bash
monerodollard --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `monerodollar-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only MoneroDollar's hash tests (after building):
...
To run only MoneroDollar's performance tests (after building):
...
To run only MoneroDollar's unit tests (after building):
