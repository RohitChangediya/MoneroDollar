...
is made or until `monerodollard` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `monerodollar-wallet-cli` and
`monerodollar-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `monerodollard` the ip/port of a socks proxy provided by a
...
which tells `monerodollard` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `monerodollard` the inbound address, network
...
which tells `monerodollard` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28083" and forwarded to `monerodollard`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000" and
forwarded to `monerodollard` localhost port 30000.
...
`monerodollard` RPC port - which is independent from the configuration for incoming
P2P anonymity connections. The anonymity network (Tor/i2p) is
[configured in the same manner](#configuration), except the localhost port
must be the RPC port (typically 15041 for mainnet) instead of the p2p port:

```
HiddenServiceDir /var/lib/tor/data/monerodollar
HiddenServicePort 15041 127.0.0.1:15041
```
...
This will store key information in `/var/lib/tor/data/monerodollar` and will forward
"Tor port" 28083 to port 28083 of ip 127.0.0.1. The file
`/usr/lib/tor/data/monerodollar/hostname` will contain the ".onion" address for use
...
Run `monerodollard` as often as possible to conceal when transactions are being sent.
...
`monerodollard` currently selects two outgoing connections every 5 minutes for
...
`monerodollard` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `monerodollard` should likely disconnect
