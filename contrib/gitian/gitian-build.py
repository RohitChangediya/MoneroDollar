...
gsigs = 'https://github.com/monerodollar-project/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if not os.path.isdir('monerodollar'):
        subprocess.check_call(['git', 'clone', args.url, 'monerodollar'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/monerodollar/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'monerodollar='+args.commit, '--url', 'monerodollar='+args.url, infile])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-linux', '--destination', '../sigs/', infile])
        subprocess.check_call('mv build/out/monerodollar-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/monerodollar/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/monerodollar/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/monerodollar-project/monerodollar', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/monerodollar', exist_ok=True)
    os.chdir('builder/inputs/monerodollar')
    if args.pull:
