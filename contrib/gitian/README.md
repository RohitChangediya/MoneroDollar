...
*Setup instructions for a Gitian build of MoneroDollar.*

Gitian is the deterministic build process that is used to build the MoneroDollar CLI
...
```bash
cp monerodollar/contrib/gitian/gitian-build.py .
```
...
fork the [gitian.sigs repository](https://github.com/monerodollar-project/gitian.sigs) and clone it on your host machine, 
or pass the signed assert file back to your build machine.

```bash
git clone git@github.com:monerodollar-project/gitian.sigs.git
...
You can also look in the [gitian.sigs](https://github.com/monerodollar-project/gitian.sigs/) repo and / or [getmonerodollar.org release checksums](https://web.getmonerodollar.org/downloads/hashes.txt) to see if others got the same checksum for the same version tag.  If there is ever a mismatch -- **STOP! Something is wrong**.  Contact others on IRC / github to figure out what is going on.
...
gpg --detach-sign ${VERSION}-linux/${GH_USER}/monerodollar-linux-*-build.assert
gpg --detach-sign ${VERSION}-win/${GH_USER}/monerodollar-win-*-build.assert
gpg --detach-sign ${VERSION}-osx/${GH_USER}/monerodollar-osx-*-build.assert
gpg --detach-sign ${VERSION}-android/${GH_USER}/monerodollar-android-*-build.assert
```
...
[monerodollar-project/gitian.sigs](https://github.com/monerodollar-project/gitian.sigs/) repository:
...
**Note:** Please ensure your gpg public key is available to check signatures by adding it to the [gitian.sigs/gitian-pubkeys/](https://github.com/monerodollar-project/gitian.sigs/tree/master/gitian-pubkeys) directory in a pull request.
...
```bash
./gitian-build.py --detach-sign --no-commit --url https://github.com/monerodollarmooo-monerodollar/bitmonerodollar -b monerodollarmooo 1f5680c8db8f4cc7acc04a04c724b832003440fd
```
